...
function Section({
  ...
  return (
    <div className="rounded-lg border border-ink-200 bg-white">
      <div
        className={cn(
          "flex items-center gap-2 border-b border-ink-100 px-4 py-2.5 text-xs font-semibold",
          accent ?? "text-ink-700",
        )}
      >
        {icon}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        >
          {rd.driver.firstName[0]}
          {rd.driver.lastName[0]}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-ink-900">
              {rd.driver.firstName} {rd.driver.lastName}
            </span>
            {isBest && (
          ...
          <div className="text-[11px] text-ink-500">
            {rd.driver.currentCity} | {rd.driver.truckType}
          </div>
        </div>
      ...
      {isBest && onAssign && (
        <button onClick={onAssign} className="btn-primary mt-3 w-full text-xs">
      ...
      <div className="card flex h-full items-center justify-center p-8 text-center">
        <div>
          ...
          <div className="mt-1 text-xs text-ink-500">
            Select a live load to run dispatch scoring and view DB-backed risk analysis.
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex-1 space-y-3 overflow-auto p-4 scrollbar-thin">
        <div className="grid grid-cols-4 gap-2 text-center text-[11px]">
          ...
          <div className="rounded-lg bg-ink-50 p-2">
            ...
            <div className="truncate text-sm font-semibold text-ink-800">{load.commodity}</div>
          </div>
        </div>

        {loading && (
          <div className="rounded-lg border border-brand-200 bg-brand-50 px-3 py-2 text-xs text-brand-700">
            Running live analysis against InsForge and Gemini.
          </div>
        )}
                ...
                <div className="rounded-lg border border-ink-200 p-2.5">
                  ...
                  <div className="mt-0.5 flex items-center gap-1">
                    <span className="text-[11px] text-ink-400 line-through">
                      {new Date(detention.originalEta).toLocaleTimeString("en-US", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </span>
                    ...
                    <span className="font-semibold text-rose-600">
                      {new Date(detention.updatedEta).toLocaleTimeString("en-US", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </span>
                ...
                <div className="rounded-lg border border-ink-200 p-2.5">
                  ...
                  <div
                    className={cn(
                      ...
                      detention.onTimeFeasible ? "text-emerald-600" : "text-rose-600",
                    )}
                  >
                ...
                <div className="rounded-lg border border-ink-200 p-2.5">
                  ...
                  <div className="mt-0.5">
                    ...
                    <span
                      className={cn(
                        "font-semibold",
                        detention.hosImpact.violationRisk ? "text-rose-600" : "text-amber-600",
                      )}
                    >
        ...
        {parking && (
          ...
          >
            <div className="space-y-2 text-xs">
              ...
              <div className="rounded-md bg-ink-50 p-2.5 leading-relaxed text-ink-600">
                {parking.explanation}
              </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-ink-800">
                      Primary: {parking.primaryStop.name}
                    </span>
                  </div>
                  <div className="mt-1 pl-5 text-[11px] text-ink-500">
                    {parking.primaryStop.city} | {parking.primaryStop.occupancyPercent}% occupied |{" "}
                    {parking.primaryStop.totalSpaces} spaces
                  </div>
                </div>
              )}
                ...
                <div className="rounded-lg border border-emerald-200 bg-emerald-50/50 p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-emerald-800">
                      Backup: {parking.backupStop.name}
                    </span>
                  </div>
                  <div className="mt-1 pl-5 text-[11px] text-emerald-700">
                    {parking.backupStop.city} | {parking.backupStop.occupancyPercent}% occupied |{" "}
                    {parking.backupStop.totalSpaces} spaces
                  </div>
                  {onReserveParking && (
                    <button onClick={onReserveParking} className="btn-primary mt-2 w-full text-xs">
              ...
              <div className="border-t border-ink-200 pt-1.5">
                ...
                <div className="mt-1 flex items-center justify-between font-semibold">
                  ...
                  <span className={cost.estimatedMargin > 0 ? "text-emerald-600" : "text-rose-600"}>
                    ${cost.estimatedMargin.toFixed(0)} (
                    {((cost.estimatedMargin / Math.max(cost.revenue, 1)) * 100).toFixed(0)}%)
                  </span>
                </div>
...
function CostRow({
  ...
  return (
    <div className={cn("flex items-center justify-between", bold && "font-semibold")}>
      ...
      <span className={cn("text-ink-800", warn && "font-semibold text-rose-600")}>{value}</span>
    </div>
  );
}
